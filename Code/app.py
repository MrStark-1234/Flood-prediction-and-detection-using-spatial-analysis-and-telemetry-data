...
def get_predicts():
    try:
        ...
        print(final)
        input_data_reshaped = np.expand_dims(np.array(final), axis=-1)
        input_data_reshaped = np.expand_dims(input_data_reshaped, axis=0) 
        predicti = model.predict(input_data_reshaped)
        print(predicti[0][0])
        # final[4] *= 15
        ...
        return render_template('predicts.html', cityname="Information about " + cityname, cities=cities, temp=round(final[0], 2), maxt=round(final[1], 2), wspd=round(final[2], 2), humidity=round(final[3], 2), pred = pred, lati=latitude, lng=longitude)
    except:
